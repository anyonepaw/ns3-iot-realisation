...
//                            Ipv4Address nodeIp4Address = Inet6SocketAddress::ConvertFrom(from).GetIpv6 ();
//
...
//        NS_LOG_INFO("Node " << GetNode ()->GetId () << " sent a " << getMessageName(responseMessage) << " message: " << buffer.GetString());
//
//        Ipv4Address outgoingIpv4Address = Inet6SocketAddress::ConvertFrom(outgoingAddress).GetIpv6 ();
...
//            m_peersSockets[outgoingIpv4Address]->Connect (Inet6SocketAddress (outgoingIpv4Address, m_commPort));
//        }
...
//    }
    ...
    void
    IotSensorNode::SendMessage(enum Messages receivedMessage, enum Messages responseMessage, std::string packet,
