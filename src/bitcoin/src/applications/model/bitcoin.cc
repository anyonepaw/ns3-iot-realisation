...
// }
    ...
    Blockchain::GetLongestForkSize(void) {
        ...
        for (blockHeight_it = m_blocks.begin(); blockHeight_it < m_blocks.end(); blockHeight_it++) {
            ...
            if (blockHeight_it->size() > 1 && forkedBlocksParentId.size() == 0) {
                ...
                for (block_it = blockHeight_it->begin(); block_it < blockHeight_it->end(); block_it++) {
                    std::map<int, int>::iterator mapIndex = forkedBlocksParentId.find(
                            block_it->GetParentBlockMinerId());
    ...
    bool operator==(const BitcoinChunk &chunk1, const BitcoinChunk &chunk2) {
        return chunk1.GetBlockHeight() == chunk2.GetBlockHeight() && chunk1.GetMinerId() == chunk2.GetMinerId() &&
               chunk1.GetChunkId() == chunk2.GetChunkId();
    }
    ...
    std::ostream &operator<<(std::ostream &out, Blockchain &blockchain) {
        ...
        for (blockHeight_it = blockchain.m_blocks.begin(), i = 0;
             blockHeight_it < blockchain.m_blocks.end(); blockHeight_it++, i++) {
            out << "  BLOCK HEIGHT " << i << ":\n";
            for (block_it = blockHeight_it->begin(); block_it < blockHeight_it->end(); block_it++) {
                out << *block_it << "\n";
            }
    ...
    const char *getMessageName(enum Messages m) {
        switch (m) {
            ...
            case EXT_GET_HEADERS:
                return "EXT_GET_HEADERS";
            case EXT_HEADERS:
                return "EXT_HEADERS";
            case EXT_GET_BLOCKS:
                return "EXT_GET_BLOCKS";
            case CHUNK:
                return "CHUNK";
            case EXT_GET_DATA:
                return "EXT_GET_DATA";
    ...
    const char *getBlockBroadcastType(enum BlockBroadcastType m) {
        switch (m) {
            ...
            case UNSOLICITED_RELAY_NETWORK:
                return "UNSOLICITED_RELAY_NETWORK";
        }
    }
